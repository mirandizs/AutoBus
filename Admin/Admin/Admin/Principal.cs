...
namespace Admin
{
    public partial class Principal : Form
    {
        ...
        public Principal(int nif, int id_utilizador)
        {
            InitializeComponent();
        ...
        private void Principal_Load(object sender, EventArgs e)
        {
            if (db.open_connection())
            {
                ...
                //btMeuPerfil.Image = Image.FromStream(new WebClient().OpenRead(URLImagem));

                using (var stream = new WebClient().OpenRead(URLImagem))
                {
                    Image originalImage = Image.FromStream(stream);
                    Bitmap resizedImage = new Bitmap(originalImage, new Size(30, 30)); // for example,
        ...
        //private async void CarregarImagemAsync()
        //{
        ...
        private void btEditarUtilizadores_Click(object sender, EventArgs e)
        {
            if (dataGridViewUtilizador.SelectedRows.Count > 0) // Verifica se há uma linha selecionada
            {
                ...
                string password = dataGridViewUtilizador.SelectedRows[0].Cells["password"].Value.ToString();
                ...
                using (Utilizador utilizador = new Utilizador("editar", user)) // `using` garante que o objeto será descartado
                {
                    this.Hide();
        ...
        private void CarregarUtilizadores(string nome = "")
        {
            if (db.open_connection())
            {
                ...
                if (!dataGridViewUtilizador.Columns.Contains("id_utilizador")) // ✔️ Evita adicionar colunas repetidas
                {
                    ...
                    dataGridViewUtilizador.Columns.Add("password", "Password");
